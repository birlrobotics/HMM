...
def main():
    ...
    model_save_path = "/home/ben/ML_data/REAL_HIRO_ONE_SA_SUCCESS/model/wrench/trails"+str(success_num)

    figure_save_path = "/home/ben/ML_data/REAL_HIRO_ONE_SA_SUCCESS/figure/wrench/trails"+str(success_num)

    result_save_path = "/home/ben/ML_data/REAL_HIRO_ONE_SA_SUCCESS/result/wrench/trails"+str(success_num)
    ...
    path_index_name = []
    for i in range(1,success_full_num+1):
        if i+1 <= 9:
            post_str = '0'+str(i+1)
        else:
            post_str = str(i+1)
    ...
    random_sample = random.sample(range(success_full_num), success_num)
    ...
    for o in range(2):
        ...
        if o == 0:
            K_Fold_1 = random_sample[:success_num/2]
            K_Fold_2 = random_sample[success_num/2:]
            
        else:
            K_Fold_2 = random_sample[:success_num/2]
            K_Fold_1 = random_sample[success_num/2:]
            
        success_num_train = np.asarray(K_Fold_1).shape[0]
        success_num_test = np.asarray(K_Fold_2).shape[0]
